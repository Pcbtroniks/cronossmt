...
class Computo extends MY_Controller {
	...
	private $form_rules = [
		...
		[
			'field' => 'memory_ram_max',
			'label' => 'Memoria Soportada',
			'rules' => 'required|integer'
		],

		[
			'field' => 'number_slot',
			'label' => 'Numero de slots',
			'rules' => 'required|integer'
		],
	...
	{
		parent::__construct();
		$this->load->model('inventory/computo_model');
	...
	public function index()
	{
		...
		($this->acl_permits('inventory.list_records')) ? $this->twig->display('inventory/computo/computo') : show_404();
	}
	...
	public function add_equipment()
	{
		...
		if($this->form_validation->run() === true)
		{
			...
			$add_equipment = $this->computo_model->add_equipment();
	...
	public function edit_equipment($equipment_id = null)
	{
		...
		if($equipment_id)
		{
			...
			if($this->form_validation->run() === true)
			{
				...
				$edit_equipment = $this->computo_model->edit_equipment($equipment_id);
	...
	public function list_equipment()
	{
		...
		$result = ['data' =>[]];
		$data = $this->computo_model->list_equipment();
	...
	{
		# Si no tiene permisos, rechazar acceso.
		($this->acl_permits('inventory.view')) ? '' : show_404();

		if ($id == null) {
			return null;
		}

		$data = $this->computo_model->get_equipment(intval($id));
		...
		$result = [
			...
			'memory_ram' => $data['memory_ram'] ?? null,
			'memory_ram_max' => $data['memory_ram_max'] ?? null,
			'number_slot' => $data['number_slot'] ?? null,
	...
	public function select2_customers()
	{
		...
		$result = $this->computo_model->select2($id, $text, $table, $searchTerm);
