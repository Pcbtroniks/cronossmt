...
class Projector extends MY_Controller {
	...
	{
		parent::__construct();
		$this->load->model('inventory/projector_model');
	...
	public function index()
	{
		...
		($this->acl_permits('inventory.list_records')) ? $this->twig->display('inventory/projector/projector') : show_404();
	}
	...
	public function add_equipment()
	{
		...
		if($this->form_validation->run() === true)
		{
			...
			$add_equipment = $this->projector_model->add_equipment();
	...
	public function edit_equipment($equipment_id = null)
	{
		...
		if($equipment_id)
		{
			...
			if($this->form_validation->run() === true)
			{
				...
				$edit_equipment = $this->projector_model->edit_equipment($equipment_id);
	...
	public function list_equipment()
	{
		...
		$result = ['data' =>[]];
		$data = $this->projector_model->list_equipment();
	...
	public function get_equipment($id = null)
	{
		...
		$data = $this->projector_model->get_equipment(intval($id));
	...
	public function select2_customers()
	{
		...
		$result = $this->projector_model->select2($id, $text, $table, $searchTerm);
