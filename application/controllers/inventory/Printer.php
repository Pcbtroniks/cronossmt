...
class Printer extends MY_Controller {
	...
	{
		parent::__construct();
		$this->load->model('inventory/printer_model');
	...
	public function index()
	{
		...
		($this->acl_permits('inventory.list_records')) ? $this->twig->display('inventory/printer/printer') : show_404();
	}
	...
	public function add_equipment()
	{
		...
		if($this->form_validation->run() === true)
		{
			...
			$add_equipment = $this->printer_model->add_equipment();
	...
	public function edit_equipment($equipment_id = null)
	{
		...
		if($equipment_id)
		{
			...
			if($this->form_validation->run() === true)
			{
				...
				$edit_equipment = $this->printer_model->edit_equipment($equipment_id);
	...
	public function list_equipment()
	{
		...
		$result = ['data' =>[]];
		$data = $this->printer_model->list_equipment();
	...
	public function get_equipment($id = null)
	{
		...
		$data = $this->printer_model->get_equipment(intval($id));
		...
		$result = [
			...
			'type' => $data['type'] ?? null,
			'multifuncional_type' => $data['multifuncional_type'] ?? null,
	...
	public function select2_customers()
	{
		...
		$result = $this->printer_model->select2($id, $text, $table, $searchTerm);
